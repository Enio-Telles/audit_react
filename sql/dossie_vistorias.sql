WITH total AS (
    SELECT
        ...
        t.status,
        t.dt_vistoria,
        t.modalidade_id || ' - ' || CONVERT(m.nome, 'US7ASCII', 'WE8MSWIN1252') modalidade,
        t.dsf,
        t.processo,
        CONVERT(ps.no_razao_social, 'US7ASCII', 'WE8MSWIN1252') solicitante,
        CONVERT(p.no_razao_social, 'US7ASCII', 'WE8MSWIN1252') auditor,
        NULL autos
    ...
    SELECT
        ...
        CASE
            ...
            WHEN df.it_co_situacao_diligencia = '04' THEN '04 - DIL. CONCLUIDA'
            WHEN df.it_co_situacao_diligencia = '05' THEN '05 - DIL. EXCLUIDA'
        END status,
        TO_DATE(df.it_da_lancamento, 'yyyymmdd') dt_vistoria,
        CONVERT(dft.it_nu_documento_origem, 'US7ASCII', 'WE8MSWIN1252') modalidade,
        NULL dsf,
        dft.it_nu_diligencia processo,
        NULL solicitante,
        su.it_co_matricula_usuario || ' - ' || CONVERT(su.it_no_usuario, 'US7ASCII', 'WE8MSWIN1252') auditor,
        da.autos autos
...
SELECT
    ...
    t.autos,
    CASE
        WHEN d.empresa_vistoria_id IS NOT NULL THEN 'RELATORIO_ASSINADO'
        ELSE NULL
    END relatorio
FROM total t

...
from utilitarios.dossie_cache_keys import VERSAO_PADRAO_DOSSIE
from utilitarios.dossie_dataset_reuse import criar_metadata_dataset_compartilhado
from utilitarios.dossie_dataset_reuse import DatasetCompartilhadoDossie
from utilitarios.dossie_dataset_reuse import carregar_dataset_reutilizavel
from utilitarios.dossie_dataset_reuse import salvar_dataset_compartilhado
from utilitarios.dossie_dataset_reuse import obter_caminho_dataset_compartilhado
from utilitarios.dossie_resolution import resolver_secao_dossie
from utilitarios.dossie_section_builder import compor_secao_dossie
...
def _extrair_chaves_funcionais_contato(dataframe: pl.DataFrame) -> set[str]:
    """Gera chaves funcionais agregadas por entidade para comparar estrategias."""
    ...
    colunas_identificacao = [
        coluna
        for coluna in ("tipo_vinculo", "cpf_cnpj_referencia", "nome_referencia", "crc_contador")
        if coluna in dataframe.columns
    ]
    if not colunas_identificacao:
        return set()

    colunas_contato = [
        coluna
        for coluna in ("telefone", "telefone_nfe_nfce", "email", "endereco", "situacao_cadastral")
        if coluna in dataframe.columns
    ]

    # A comparacao funcional precisa refletir a agenda integrada por entidade,
    # e nao apenas a cardinalidade das linhas por fonte Oracle.
    agregacoes = [
        _agregar_valores_funcionais_contato(coluna).alias(coluna)
        for coluna in colunas_contato
    ]
    if not agregacoes:
        agregacoes = [pl.lit("").alias("_sem_campos_contato")]

    chaves: set[str] = set()
    dataframe_agregado = (
        dataframe
        .group_by(colunas_identificacao, maintain_order=True)
        .agg(agregacoes)
        .sort(colunas_identificacao)
    )
    colunas_chave = [*colunas_identificacao, *[coluna for coluna in colunas_contato if coluna in dataframe_agregado.columns]]
    for linha in dataframe_agregado.select(colunas_chave).to_dicts():
...
def _agregar_valores_funcionais_contato(coluna: str) -> pl.Expr:
    """Consolida valores textuais unicos por entidade para comparacao funcional."""
    ...
    return (
        ...
        .fill_null("")
        .str.strip_chars()
        .filter(pl.col(coluna).cast(pl.Utf8, strict=False).fill_null("").str.strip_chars().ne(""))
        .unique()
        .sort()
        .str.join(" || ")
    )

...
def _coluna_texto_forcado(coluna: str, cfg: dict[str, Any]) -> bool:
    return coluna in cfg["texto_forcado"] or is_excel_text_identifier_column_name(coluna)


def _preparar_dataframe_para_excel(
    df_pd: pd.DataFrame,
    cfg: dict[str, Any],
) -> pd.DataFrame:
    df_preparado = df_pd.copy()

    for coluna in df_preparado.columns:
        col_lower = str(coluna).strip().lower()

        if _coluna_texto_forcado(col_lower, cfg):
            df_preparado[coluna] = df_preparado[coluna].map(
                formatar_identificador_excel_texto
            )
            continue

        eh_data_hora = (
            col_lower in cfg["datetime_cols"]
            or is_excel_datetime_column_name(col_lower)
        )
        eh_data = (
            col_lower in cfg["date_cols"]
            or is_excel_date_column_name(col_lower)
        )

        if eh_data_hora or eh_data:
            serie_convertida = pd.to_datetime(df_preparado[coluna], errors="coerce")
            if serie_convertida.notna().any():
                df_preparado[coluna] = (
                    serie_convertida
                    if eh_data_hora
                    else serie_convertida.dt.date
                )
...
def _escolher_formato(col_lower: str, dtype: str, cfg: dict[str, Any], formatos: dict[str, Any]):
    if is_year_column_name(col_lower):
        return formatos["ano"]
    if col_lower in cfg["datetime_cols"] or is_excel_datetime_column_name(col_lower) or "datetime64" in dtype:
        return formatos["data_hora"]
    if col_lower in cfg["date_cols"] or is_excel_date_column_name(col_lower):
...
def exportar_excel(
    ...
    df_pd = _preparar_dataframe_para_excel(df_pd, cfg)
    ...
    with pd.ExcelWriter(
        ...
        for col_idx, col_name in enumerate(df_pd.columns):
            ...
            if largura is None:
                ...
                elif _coluna_texto_forcado(col_lower, cfg) or col_lower in cfg["url_cols"]:
                    largura = _largura_auto(df_pd[col_name], col_name, minimo=12, maximo=42)
                elif (
                    col_lower in cfg["date_cols"]
                    or col_lower in cfg["datetime_cols"]
                    or is_excel_date_column_name(col_lower)
                    or is_excel_datetime_column_name(col_lower)
                ):

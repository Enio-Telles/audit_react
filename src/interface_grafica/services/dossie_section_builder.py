    ...
    for linha in _iterar_registros(contadores):
        ...
        linhas.append(
            _registro_contato_base(
                ...
                situacao_cadastral=_normalizar_texto(linha.get("situacao")),
...
def _normalizar_registro_contador(origem: str, linha: dict[str, object]) -> dict[str, object] | None:
    ...
    if origem == "dossie_contador.sql":
        ...
        return {
            ...
            "nome": nome,
            "situacao": _normalizar_texto(linha.get("situacao")),
    ...
    if origem == "dossie_historico_fac.sql":
        ...
        return {
            ...
            "prioridade_origem_contador": PRIORIDADE_FONTE_CONTADOR.get(origem, 99),
            "origem_contador": origem,
            "tabela_origem_contador": "SITAFE.SITAFE_HISTORICO_CONTRIBUINTE; SITAFE.SITAFE_PESSOA; BI.DM_PESSOA",
        }
...
def _linhas_socios(cnpj_consultado: str, cnpj_raiz: str, socios: pl.DataFrame) -> list[dict[str, object]]:
    ...
    for linha in _iterar_registros(socios):
        ...
        situacao_normalizada = unicodedata.normalize("NFKD", situacao).upper()
        if "ATUAL" not in situacao_normalizada:
            continue
        linhas.append(
            _registro_contato_base(
                ...
                tabela_origem="SITAFE.SITAFE_HISTORICO_SOCIO; BI.DM_PESSOA",
                ordem_exibicao=40,
            )
        )
    return linhas

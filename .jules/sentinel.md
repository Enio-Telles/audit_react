...
## 2025-02-24 - [Fix Path Traversal in SPA Fallback]
**Vulnerability:** A path traversal vulnerability existed in the SPA fallback route `/{path:path}` in `server/python/api.py`. The handler checked if the path started with `assets/` and concatenated it directly with `BUILD_DIR` without sanitization, allowing arbitrary file reading via `assets/../../../../etc/passwd`.
**Learning:** This occurred because naive string prefix checking (`startswith`) is insufficient for securing dynamically concatenated file paths against `../` resolution sequences.
**Prevention:** Always resolve the absolute path using `.resolve()` and enforce boundary checking with `.is_relative_to(BASE_DIR.resolve())` before validating existence or serving the file. Wrap the resolution in a `try-except` block to prevent server crashes from invalid OS-level paths.

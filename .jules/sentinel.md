...
## 2024-04-16 - Path Traversal via startswith()
**Vulnerability:** Path validation in `create_sql_file` used `.startswith()` which allows path traversal bypass (e.g., `/app/sql_fake` bypassing `/app/sql`).
**Learning:** String matching on paths is insecure.
**Prevention:** Always use `Path.is_relative_to()` to ensure path containment.

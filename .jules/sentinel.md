...
## 2024-05-26 - Path Traversal via String Matching Bypass
**Vulnerability:** Path validation using `str(path).startswith(str(base))` allows traversal because a path like `/app/sql_fake` starts with `/app/sql` but is outside the base directory.
**Learning:** String matching on resolved paths is insufficient for security validation as it ignores path boundaries.
**Prevention:** Always use `.is_relative_to(safe_base.resolve())` to strictly enforce path containment within a safe base directory.

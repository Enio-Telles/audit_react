...
def test_get_resumo_comparacoes_contato_consolida_historico(tmp_path, monkeypatch):
                ...
                json.dumps(
                    {
                        ...
                        "comparacao_estrategia_alternativa": {
                            ...
                            "quantidade_chaves_faltantes": 2,
                            "quantidade_chaves_extras": 1,
                            "amostra_chaves_faltantes": [
                                "SOCIO_ANTIGO|11111111111|Socio Antigo|",
                            ],
                            "amostra_chaves_extras": [
                                "EMPRESA_FAC_ATUAL|12345678000190|Empresa Base|",
                            ],
                            "campos_criticos_atual": {
                                "telefone": 3,
                                "email": 1,
                            },
                            "campos_criticos_referencia": {
                                "telefone": 2,
                                "email": 2,
                            },
                        },
    ...
    assert resposta.ultimaEstrategiaReferencia == "composicao_polars"
    assert resposta.ultimaSqlPrincipalReferencia == "dados_cadastrais.sql"
    assert resposta.ultimoStatusComparacao == "convergencia_basica"
    assert resposta.ultimoCacheKey == "a3"
    assert resposta.ultimoTotalChavesFaltantes == 2
    assert resposta.ultimoTotalChavesExtras == 1
    assert resposta.ultimaAmostraChavesFaltantes == [
        "SOCIO_ANTIGO|11111111111|Socio Antigo|"
    ]
    assert resposta.ultimaAmostraChavesExtras == [
        "EMPRESA_FAC_ATUAL|12345678000190|Empresa Base|"
    ]
    assert resposta.ultimosCamposCriticosAtual == {"telefone": 3, "email": 1}
    assert resposta.ultimosCamposCriticosReferencia == {"telefone": 2, "email": 2}
    assert resposta.historyFile.endswith("historico_comparacao_contato_12345678000190.jsonl")
...
def test_post_relatorio_comparacoes_contato_materializa_markdown(tmp_path, monkeypatch):
    ...
    assert "dados_cadastrais.sql" in resposta.content
    assert "Ultimas chaves faltantes: `3`" in resposta.content
    assert "Amostra de chaves faltantes" in resposta.content
    assert "SOCIO_ANTIGO|11111111111|Socio Antigo|" in resposta.content
    assert "`telefone`: atual=`3` | referencia=`2` | delta=`1`" in resposta.content
    assert "`email`: atual=`1` | referencia=`2` | delta=`-1`" in resposta.content

...
const navItems: NavItem[] = [
  { path: "/dashboard", label: "Dashboard", icon: LayoutDashboard, description: "Visao geral e atalhos" },
  { path: "/dados-cadastrais", label: "Dados Cadastrais", icon: IdCard, description: "Ficha cadastral do documento em analise" },
  { path: "/extracao", label: "Extracao", icon: Database, description: "Extracao Oracle e pipeline" },
  { path: "/consulta", label: "Consulta", icon: Table2, description: "Visualizar tabelas Parquet" },
  { path: "/agregacao", label: "Agregacao", icon: Boxes, description: "Agrupar produtos" },
  { path: "/conversao", label: "Conversao", icon: ArrowLeftRight, description: "Fatores e unidades" },
  { path: "/estoque", label: "Estoque", icon: Package, description: "Movimentacao e saldos" },
  { path: "/mapeamento-oracle", label: "Mapa Oracle", icon: GitBranch, description: "Raiz SQL, Parquets e Polars" },
  { path: "/relatorios", label: "Relatorios", icon: FileText, description: "Relatorios fiscais conclusivos" },
];

const bottomNavItems: NavItem[] = [
  { path: "/", label: "Entrada", icon: Search, description: "Selecionar CNPJ, CPF ou lote" },
  { path: "/configuracoes", label: "Configuracoes", icon: Settings, description: "Preferencias do sistema" },
];
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const currentPage = [...navItems, ...bottomNavItems].find((item) => item.path === location) ?? navItems[0];
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      >
          ...
          <Link href="/" className={cn("flex items-center gap-3", collapsed && "justify-center")}>
            ...
            {!collapsed ? (
              <div className="flex min-w-0 flex-col">
                <span className="truncate text-sm font-bold tracking-tight text-sidebar-foreground">Audit React</span>
                <span className="text-[10px] font-medium uppercase tracking-wider text-sidebar-foreground/50">SEFIN</span>
              </div>
            ) : null}
          </Link>
        </div>
        ...
        <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto px-2 py-3">
          {navItems.map((item) => (
            <NavLink key={item.path} item={item} collapsed={collapsed} isActive={location === item.path} />
          ))}
        </nav>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-border bg-background px-6">
          <div>
            <h1 className="text-base font-bold tracking-tight text-foreground">{currentPage.label}</h1>

...
import type {
  ...
  ManifestacoesRelatorio,
} from "@/types/relatorio";

const OPCOES_MANIFESTACAO: Array<{
  chave: keyof ManifestacoesRelatorio;
  rotulo: string;
}> = [
  {
    chave: "regularizou_integralmente",
    rotulo: "Regularizou integralmente as pendencias",
  },
  ...
  {
    chave: "nao_apresentou_manifestacao",
    rotulo: "Nao apresentou manifestacao",
  },
];
...
const AUDITOR_VAZIO: DadosAuditor = {
  ...
  endereco:
    "Avenida Farquar, n. 2986 - Palacio Rio Madeira - Bairro Pedrinhas - CEP 76.801-470 - Porto Velho/RO",
  local_data: "",
};
...
function resumirManifestacoes(manifestacoes: ManifestacoesRelatorio): string {
  const marcadas = OPCOES_MANIFESTACAO.filter(
    opcao => manifestacoes[opcao.chave]
...
export default function Relatorios() {
  ...
  const [cnpjsComRelatorio, setCnpjsComRelatorio] = useState<
    CnpjComRelatorio[]
  >([]);
  const [diagnostico, setDiagnostico] = useState<DiagnosticoRelatorios | null>(
    null
  );
  ...
  const motivoModeloIndividual =
    modeloIndividual?.mensagem || "Modelo Word individual indisponivel.";
  const motivoModeloGeral =
    modeloGeral?.mensagem || "Modelo Word geral indisponivel.";
  ...
  const handleCarregarRelatorio = useCallback(
    async (cnpj: string) => {
      ...
      try {
        ...
        const arquivosSelecionados = detsList
        ...
        if (dados && dados.contribuinte) {
          setRelatorio({
            ...
            arquivos_notificacao_incluidos: dados.arquivos_notificacao_incluidos
              ?.length
  ...
  useEffect(() => {
    if (
      !selecaoOperacional ||
      selecaoOperacional.cnpjs_resolvidos.length === 0
    ) {
      return;
    }
  ...
  const handleSalvarRelatorio = async () => {
    ...
    try {
      const relatorioSalvo = await api.salvarRelatorio(
        cnpjSelecionado,
        relatorio
      );
      setRelatorio({
  ...
  const handleUploadDet = async (
    ...
    try {
      ...
      setRelatorio(atual => ({
        ...atual,
        arquivos_notificacao_incluidos: detsList
          .filter(item => item.selecionado)
          .map(item => item.nome),
      }));
      const lista = await api.listarCnpjsComRelatorio();
  ...
  const handleAlternarDet = (nomeArquivo: string, marcado: boolean) => {
    setRelatorio(atual => {
      const nomes = marcado
        ? Array.from(
            new Set([...atual.arquivos_notificacao_incluidos, nomeArquivo])
          )
        : atual.arquivos_notificacao_incluidos.filter(
  ...
  const handleGerarPdfGeral = async () => {
    ...
    try {
      await api.gerarPdfGeral(
        dsfAtiva || undefined,
        dsfCnpjs.length > 0 ? dsfCnpjs : undefined
      );
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <p className="text-xs text-muted-foreground">
            Geracao de relatorios de analise Fisconforme nao cumprido por CNPJ e
            consolidado por DSF
          </p>
        </CardHeader>
      </Card>
        ...
        <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded border p-3">
            ...
            <p className="mt-1 text-sm font-semibold">
              {prontoPdf ? "Pronta" : "Dependencias ausentes"}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              {prontoPdf
                ? `Backend pronto com: ${dependenciasAtivas.join(", ")}`
                : motivoPdfIndisponivel}
            </p>
          </div>
          <div className="rounded border p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
            ...
            <p className="mt-1 text-sm font-semibold">
              {modeloIndividual?.pronto && modeloGeral?.pronto
                ? "Individual e geral ok"
                : "Verificar templates"}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Individual: {modeloIndividual?.existe ? "ok" : "ausente"} | Geral:{" "}
              {modeloGeral?.existe ? "ok" : "ausente"}
            </p>
          </div>
          <div className="rounded border p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
            ...
            <p className="mt-1 text-sm font-semibold">
              {diagnostico?.total_dsfs ?? dsfs.length}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Inclui DSFs persistidas e inferidas a partir dos relatorios dos
              CNPJs.
            </p>
          </div>
          <div className="rounded border p-3">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    value={auditor.nome}
                    onChange={e =>
                      setAuditor({ ...auditor, nome: e.target.value })
                    }
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    value={auditor.matricula}
                    onChange={e =>
                      setAuditor({ ...auditor, matricula: e.target.value })
                    }
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    value={auditor.cargo}
                    onChange={e =>
                      setAuditor({ ...auditor, cargo: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    value={auditor.orgao}
                    onChange={e =>
                      setAuditor({ ...auditor, orgao: e.target.value })
                    }
                  />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <Input
                  value={auditor.local_data}
                  onChange={e =>
                    setAuditor({ ...auditor, local_data: e.target.value })
                  }
                  placeholder="Porto Velho, 28 de marco de 2026"
                />
              </div>
                ...
                <ScrollArea className="h-64">
                  {dsfs.length === 0 ? (
                    <p className="py-4 text-center text-xs text-muted-foreground">
                      Nenhuma DSF efetiva encontrada
                    </p>
                  ) : (
                      ...
                      {dsfs.map(dsf => (
                        ...
                        >
                          ...
                          <div className="text-muted-foreground">
                            {dsf.descricao || "Sem descricao"}
                          </div>
                          <div className="mt-1 flex gap-1">
                            <Badge variant="outline" className="text-[10px]">
                              {dsf.cnpjs.length} CNPJs
                            </Badge>
                            <Badge
                              variant="outline"
                              className="text-[10px] capitalize"
                            >
                              {dsf.origem || "persistida"}
                            </Badge>
                          </div>
                        </button>
                      ))}
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      value={dsfNumero}
                      onChange={e => setDsfNumero(e.target.value)}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      value={dsfDescricao}
                      onChange={e => setDsfDescricao(e.target.value)}
                      placeholder="Acervo Fisconforme"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      className="font-mono"
                      onKeyDown={e => {
                        if (e.key === "Enter") handleAdicionarCnpjDsf();
                      }}
                    />
                    <Button
                      variant="outline"
                      onClick={handleAdicionarCnpjDsf}
                  ...
                  <ScrollArea className="h-40 rounded border p-2">
                    {dsfCnpjs.length === 0 ? (
                      ...
                      <div className="space-y-1">
                        {dsfCnpjs.map(cnpj => {
                          const info = cnpjsComRelatorio.find(
                            c => c.cnpj === cnpj
                          );
                          return (
                            <div
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">
                  {cnpjSelecionado
                    ? `Relatorio — ${formatarCnpj(cnpjSelecionado)}`
                    : "Selecione um CNPJ"}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      value={relatorio.ie}
                      onChange={e =>
                        setRelatorio({ ...relatorio, ie: e.target.value })
                      }
                      placeholder="IE"
                      className="font-mono"
                    />
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      value={relatorio.dsf}
                      onChange={e =>
                        setRelatorio({ ...relatorio, dsf: e.target.value })
                      }
                      placeholder="Numero da DSF"
                      className="font-mono"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      value={relatorio.notificacao_det}
                      onChange={e =>
                        setRelatorio({
                          ...relatorio,
                          notificacao_det: e.target.value,
                        })
                      }
                      placeholder="Numero da DET"
                      className="font-mono"
                    />
                  </div>
                </div>
                <div className="space-y-1.5">
                  ...
                  <div className="space-y-2 rounded border p-3">
                    <p className="text-xs text-muted-foreground">
                      Marque com X as opcoes aplicaveis ao relatorio final.
                    </p>
                    {OPCOES_MANIFESTACAO.map(opcao => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            handleAlternarManifestacao(opcao.chave)
                          }
                          className="flex w-full items-center gap-3 rounded px-2 py-1.5 text-left text-sm transition-colors hover:bg-accent/40"
                        >
                          <span
                            className={`flex h-4 w-4 items-center justify-center border text-[10px] font-bold ${ativo ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background text-transparent"}`}
                          >
                            X
                          </span>
                          <span
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Resumo: {resumirManifestacoes(relatorio.manifestacoes)}
                    </p>
                  </div>
                </div>
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    value={relatorio.contatos_realizados}
                    onChange={e =>
                      setRelatorio({
                        ...relatorio,
                        contatos_realizados: e.target.value,
                      })
                    }
                    placeholder="Descreva os contatos realizados..."
                    rows={3}
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    value={relatorio.decisao_fiscal}
                    onChange={e =>
                      setRelatorio({
                        ...relatorio,
                        decisao_fiscal: e.target.value,
                      })
                    }
                    placeholder="Descreva a decisao fiscal..."
                    rows={3}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                    Desfecho / Situacao final
                  </Label>
                  <Textarea
                    value={relatorio.desfecho}
                    onChange={e =>
                      setRelatorio({ ...relatorio, desfecho: e.target.value })
                    }
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      disabled={!cnpjSelecionado || api.loading}
                      className="gap-1"
                    >
                    ...
                    {dets.length > 0 && (
                      <Badge variant="outline" className="text-green-600">
                        <CheckCircle2 className="mr-1 h-3 w-3" /> {dets.length}{" "}
                        DET(s)
                      </Badge>
                    )}
                  </div>
                  ...
                  {dets.length > 0 && (
                    <div className="space-y-1 rounded border p-2">
                      {dets.map(det => (
                        ...
                        >
                          <Checkbox
                            checked={relatorio.arquivos_notificacao_incluidos.includes(
                              det.nome
                            )}
                            onCheckedChange={checked =>
                              handleAlternarDet(det.nome, checked === true)
                            }
                          ...
                          <span className="text-muted-foreground">
                            ({(det.tamanho_bytes / 1024).toFixed(0)} KB)
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      disabled={
                        !cnpjSelecionado ||
                        api.loading ||
                        !modeloIndividual?.pronto
                      }
                      className="gap-2"
                      title={
                        !modeloIndividual?.pronto
                          ? motivoModeloIndividual
                          : undefined
                      }
                    >
                      {api.loading ? (
                    ...
                    <Button
                      ...
                      disabled={
                        !cnpjSelecionado || api.loading || !prontoPdfIndividual
                      }
                      className="gap-2"
                      title={
                        !prontoPdf
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  Relatorio Geral Consolidado por DSF
                </h3>
                <p className="text-xs text-muted-foreground">
                  Selecione uma DSF para gerar o relatorio geral com todos os
                  CNPJs vinculados, incluindo apenas DETs locais realmente
                  disponiveis.
                </p>
                <div className="flex flex-wrap gap-2">
                  {dsfs.length === 0 ? (
                    <p className="text-xs text-muted-foreground">
                      Nenhuma DSF efetiva encontrada no storage.
                    </p>
                  ) : (
                    dsfs.map(dsf => (
                      <Button
                        key={dsf.numero}
                        variant={
                          dsfAtiva === dsf.numero ? "default" : "outline"
                        }
                        size="sm"
                        onClick={() => handleSelecionarDsf(dsf.numero)}
                        className="gap-1"
                ...
                {dsfAtiva && (
                  <div className="rounded border p-3 space-y-2">
                    <p className="text-xs font-semibold">
                      DSF {dsfAtiva} — {dsfCnpjs.length} CNPJs vinculados:
                    </p>
                      ...
                      {dsfCnpjs.map(cnpj => {
                        ...
                        return (
                          ...
                          >
                            {formatarCnpj(cnpj)}
                            {info && (
                              <CheckCircle2 className="ml-1 h-2.5 w-2.5 text-green-600" />
                            )}
                          </Badge>
                        );
                      })}
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  CNPJs com relatorio preenchido
                </h3>
                {cnpjsComRelatorio.length === 0 ? (
                  <p className="text-xs text-muted-foreground">
                    Nenhum CNPJ com relatorio preenchido. Preencha na aba CNPJ.
                  </p>
                ) : (
                  <DataTable
                    dados={cnpjsComRelatorio}
                    customColumnDefs={[
                      {
                        field: "cnpj",
                        headerName: "CNPJ",
                        cellRenderer: (params: any) =>
                          formatarCnpj(params.value),
                      },
                      { field: "contribuinte", headerName: "Contribuinte" },
                      { field: "dsf", headerName: "DSF" },
                      {
                        field: "tem_det",
                        headerName: "DET",
                        cellRenderer: (params: any) =>
                      ...
                      { field: "manifestacao", headerName: "Manifestacao" },
                      {
                        field: "acao",
                        headerName: "Acao",
                        cellRenderer: (params: any) => {
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    toast.success(
                                      `DOCX de ${item.contribuinte} gerado`
                                    );
                                  } catch (erro) {
                                    toast.error(
                                ...
                                disabled={
                                  api.loading || !modeloIndividual?.pronto
                                }
                                title={
                                  !modeloIndividual?.pronto
                                    ? motivoModeloIndividual
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    toast.success(
                                      `PDF de ${item.contribuinte} gerado`
                                    );
                                  } catch (erro) {
                                    toast.error(
                                      `Erro: ${(erro as Error).message}`

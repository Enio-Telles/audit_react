...
export default function Conversao() {
  ...
  // ⚡ Bolt: Memoize filtered list to prevent unnecessary re-calculations when editing inline
  const filteredFatores = useMemo(
    () =>
      FATORES_EXEMPLO.filter(f => {
        const matchSearch =
          f.descricao_padrao.toLowerCase().includes(searchTerm.toLowerCase()) ||

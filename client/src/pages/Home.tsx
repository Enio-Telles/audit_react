      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          <span className="font-mono text-xs font-semibold text-primary">
            {formatarCnpj(alvo.cnpj)}
          </span>
          <Badge
            variant={
              alvo.status_pipeline === "completo" ? "default" : "outline"
            }
          >
            {alvo.status_pipeline === "completo"
              ? "Completo"
              : alvo.status_pipeline === "parcial"
                ? "Parcial"
                : "Sem pipeline"}
          </Badge>
        </div>
        <p className="text-sm font-semibold">
          {alvo.contribuinte ?? "Sem nome"}
        </p>
...
export default function Home() {
  ...
  // ⚡ Bolt: Usando fallback estavel para evitar quebra de memoization quando catalogo esta carregando
  const alvos = useMemo(() => catalogo?.alvos ?? [], [catalogo?.alvos]);
  const badgeOracle = classificarStatusOracle(statusSistema);

  const cnpjDigitado = normalizarDigitos(entradaCnpj).slice(0, 14);

  // ⚡ Bolt: Otimizacao de re-render. Evita recalcular o filtro de array na thread principal
  // em cada render do dashboard, movendo a operacao toLowerCase para fora do loop.
  const alvosFiltrados = useMemo(() => {
    if (!entradaCnpj.trim()) return alvos.slice(0, 10);
    const termo = entradaCnpj.toLowerCase();
    return alvos.filter(
      (alvo: any) =>
        alvo.cnpj.includes(cnpjDigitado) ||
        (alvo.contribuinte ?? "").toLowerCase().includes(termo)
    );
  }, [entradaCnpj, alvos, cnpjDigitado]);
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
          ...
          <CardContent className="grid gap-6 p-6 lg:grid-cols-[1fr_1fr]">
            <div className="space-y-4 bg-[linear-gradient(135deg,rgba(15,23,42,0.98),rgba(30,64,175,0.92))] p-6 text-primary-foreground">
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                ...
                <Badge variant="secondary" className="bg-white/12 text-white">
                  CNPJs: {catalogo?.resumo?.total_cnpjs ?? 0}
                </Badge>
              </div>
            </div>
            ...
            <div className="space-y-4 p-6">
              ...
              <div className="grid gap-2">
                ...
                <Button
                  variant="outline"
                  className="h-auto justify-start py-3"
                  onClick={() => irParaPagina("/agregacao-conversao")}
                >
                  <Boxes className="mr-2 h-4 w-4" />
                  <div className="text-left">
                    <span className="block text-sm font-semibold">
                      Agregação e Conversão
                    </span>
                    <span className="block text-xs text-muted-foreground">
                      Agrupar produtos e fatores
                    </span>
                  </div>
                </Button>
                <Button
        ...
        <Card className="mb-6 border-border/80">
          <CardHeader>
            ...
            <CardDescription>
              Informe um CNPJ ou selecione da lista abaixo
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="mb-4 flex gap-2">
              <Input
                value={entradaCnpj}
                onChange={e =>
                  setEntradaCnpj(formatarCnpjInput(e.target.value))
                }
                placeholder="00.000.000/0000-00"
                className="max-w-sm font-mono"
              />
              <Button
                onClick={() => cnpjDigitado && selecionarCnpj(cnpjDigitado)}
              >
                Selecionar
              </Button>
            </div>

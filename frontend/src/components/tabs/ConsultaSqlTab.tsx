...
export function ConsultaSqlTab() {
  ...
  const execMutation = useMutation({
    // 🛡️ Sentinel: Send selectedPath instead of raw sql text to prevent arbitrary SQL execution
    mutationFn: () => sqlApi.execute(selectedPath, selectedCnpj ?? undefined),
    ...
    <div className="flex flex-col h-full p-3 gap-3">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        <button
          ...
          disabled={!selectedPath || execMutation.isPending}
        ...
        <button onClick={() => { setSqlText(''); setSelectedPath(''); setResult(null); setError(''); }} className={btnCls + ' bg-slate-700 hover:bg-slate-600 text-slate-200'}>
          Limpar
        </button>
        {result && (
          <ColumnToggle

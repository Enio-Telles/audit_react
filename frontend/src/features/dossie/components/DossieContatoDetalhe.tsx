...
function resumir_grupos(
  ...
  return (Object.keys(grupos) as GrupoAgenda[]).map((grupo) => {
    ...
    // Bolt Optimization: Calculate multiple aggregations in a single pass
    // instead of chaining .reduce() and .filter() calls to avoid redundant
    // O(N) traversals and repeated expensive calls to calcular_status_entidade.

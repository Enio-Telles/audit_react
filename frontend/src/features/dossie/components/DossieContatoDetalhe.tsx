...
function resumir_grupos(
  ...
  return (Object.keys(grupos) as GrupoAgenda[]).map((grupo) => {
    ...
    // ⚡ Bolt: Consolidated multiple aggregations into a single for...of loop to prevent redundant O(N) traversals and avoid calling calcular_status_entidade multiple times per entity. Expected impact: Reduced CPU usage and array allocations on large entity datasets.
    ...
    for (const entidade of entidades) {
      ...
      if (status === "divergente") totalConflitos++;
      if (status === "sem contato") totalSemContato++;
    }

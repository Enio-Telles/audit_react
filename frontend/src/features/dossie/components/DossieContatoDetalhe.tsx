...
function resumir_grupos(
  ...
  return (Object.keys(grupos) as GrupoAgenda[]).map((grupo) => {
    ...
    // ⚡ Bolt: Consolidated multiple .reduce() and .filter() calls into a single loop.
    // This prevents redundant O(N) traversals per metric and avoids duplicate calls to calcular_status_entidade().
    ...
    for (const entidade of entidades) {
      ...
      else if (status === "sem contato") totalSemContato++;
    }

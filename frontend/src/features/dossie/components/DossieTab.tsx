...
function obter_rotulo_estrategia_execucao(
  resultado: DossieSyncResponse,
): string {
...
function formatar_comparacao_resumo(
...
function montar_mensagem_sucesso(resultado: DossieSyncResponse): string {
  ...
  const sqlPrincipal = resultado.sql_principal
    ? ` - principal ${resultado.sql_principal}`
    : "";
...
export function DossieTab({ cnpj, razaoSocial }: DossieTabProps) {
  ...
  const [usarSqlConsolidadoContato, setUsarSqlConsolidadoContato] =
    useState(false);
      ...
      <div className="mb-4 rounded-2xl border border-slate-700 bg-slate-900/60 p-4">
        ...
        <label className="mt-4 flex items-start gap-3 rounded-xl border border-slate-700/80 bg-slate-950/60 p-3 text-sm text-slate-300">
          <input
            ...
            onChange={(event) =>
              setUsarSqlConsolidadoContato(event.target.checked)
            }
          ...
          <span>
            ...
            <span className="block text-xs text-slate-400">
              ...
              `dossie_contato.sql`. Quando desabilitado, permanece o fluxo
        ...
        <div className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {sections.map((section) => {
              ...
              const estrategiaResumo = formatar_estrategia_resumo(
                section.executionStrategy,
              );
              const comparacaoResumo = formatar_comparacao_resumo(
                section.alternateStrategyComparison,
              );
              ...
              const podeVisualizar =
                section.status !== "idle" || estadoLocal?.tipo === "sucesso";
              ...
              return (
                ...
                >
                  ...
                  {comparacaoResumo && (
                    <div
                      className={`mt-2 text-[11px] ${comparacaoResumo.classe}`}
                    >
                  ...
                  {comparacaoResumo &&
                    (section.alternateStrategyMissingKeys !== undefined ||
                      ...
                      <div className="mt-1 text-[11px] text-slate-500">
                        Chaves faltantes:{" "}
                        {section.alternateStrategyMissingKeys ?? 0} | Chaves
                        extras: {section.alternateStrategyExtraKeys ?? 0}
                      </div>
                    )}
                  {section.id === "contato" && (
                    <div className="mt-2 text-[11px] text-slate-500">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={
                          ...
                          estaSincronizando ||
                          mutacaoSincronizacao.isPending
                        }

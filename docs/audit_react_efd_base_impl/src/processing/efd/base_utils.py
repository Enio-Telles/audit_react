"""
Utilitários compartilhados para construção da camada base EFD.

Objetivos:
- padronizar aliases de colunas;
- localizar colunas equivalentes entre datasets extraídos;
- aplicar casts coerentes;
- escrever Parquet com metadados mínimos.

Essas funções foram feitas para reduzir duplicação entre os builders
`base__efd__*` e facilitar otimizações futuras.
"""
...
COMMON_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "dt_doc": ("dt_doc", "dt_e_s", "data_doc", "data_emissao"),
    "dt_inv": ("dt_inv", "data_inv", "data_inventario"),
    "qtd": ("qtd", "quantidade"),
...
def alias_columns(df: pl.DataFrame | pl.LazyFrame, mapping: Mapping[str, Sequence[str]]) -> pl.LazyFrame:
    ...
    for canonical, aliases in mapping.items():
        col = pick_existing(schema_names, aliases)
        if col is not None:
            used.add(col)
            exprs.append(pl.col(col).alias(canonical))
...
def write_partitioned_parquet(
    ...
    # implementação simples: separa por partição em diretórios
    for row in df.select(list(partition_cols)).unique().to_dicts():
        part_df = df
        part_path = target
        for col in partition_cols:
            value = row.get(col)
            part_df = part_df.filter(pl.col(col) == value)
            part_path = part_path / f"{col}={value}"
        part_path.mkdir(parents=True, exist_ok=True)
        part_df.write_parquet(part_path / "part-000.parquet")
    return target
...
def normalize_period_column(lf: pl.LazyFrame) -> pl.LazyFrame:
    ...
    if "dt_doc" in lf.collect_schema().names():
        return lf.with_columns(
            pl.col("dt_doc")
            .cast(pl.Utf8)
            .str.replace_all("-", "")
    ...
    if "dt_inv" in lf.collect_schema().names():
        return lf.with_columns(
            pl.col("dt_inv")
            .cast(pl.Utf8)
            .str.replace_all("-", "")
